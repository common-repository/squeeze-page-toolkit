 ...
 * V0.10 - 11th Apr 2018 - Added HTML attributes support (e.g. for placeholders, data attributes)
 * V0.11 - 16th May 2018 - Added default parameter for getArrayValue()
 *                       - Added debugmode flag
 *                       - Added support for passing an array to afterSaveFunction for object methods.
 * V0.12 - 25th Jul 2019 - Added support for passing an array to filterBeforeSaveFunction for object methods.
 * 
 * 
...
if (!class_exists('EasyForm')) { class EasyForm
{
	...
	/**
	 * If true, show extra debug information.
	 * @var boolean
	 */
	public $debugMode;
	...
	protected function createElementObject($fieldName, $fieldDetails) 
	{
		...
		if ($type != 'break') 
		{
			...
			// Exta attributes for the field.
			if ($attributes = $this->formObj->getArrayValue($fieldDetails, 'attributes')) {
	...
	protected function processPost()
	{
		...
		if ($this->formObj->formSubmitted())
		{
			...
			if ($this->formObj->formValid()) 
			{
				...
				if ($this->filterBeforeSaveFunction && is_callable($this->filterBeforeSaveFunction)) 
				{
				    // DJH 2019-07-25 - Extended to allow for passing an array
				    // e.g.  $settings->filterBeforeSaveFunction = array(&$this, '__page__Settings_beforeSaveFns');
				    // or e.g. $settings->filterBeforeSaveFunction = '__page__Settings_beforeSaveFns';
				    
				    // It was this: 
					// $formValues = call_user_func($this->filterBeforeSaveFunction, $originalFormValues, $this);
					
					if (is_array($this->filterBeforeSaveFunction)) {
					    $formValues = call_user_func_array($this->filterBeforeSaveFunction, array($originalFormValues, $this));
					} else {
					    $formValues = call_user_func($this->filterBeforeSaveFunction, $originalFormValues, $this);
					}
				...
				if ($this->afterSaveFunction && is_callable($this->afterSaveFunction)) 
				{
				    //
				    // DJH 2018-05-16 - Extended to allow for passing an array
				    // e.g.  $settings->afterSaveFunction = array(&$this, '__page__Settings_afterSaveFns');
				    // or e.g. $settings->afterSaveFunction = '__page__Settings_afterSaveFns';
				    //
				    if (is_array($this->afterSaveFunction)) {
				        call_user_func_array($this->afterSaveFunction, array($formValues, $originalFormValues, $this));
				    } else {
